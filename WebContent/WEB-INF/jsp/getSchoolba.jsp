		...
		<div class="col-md-12 column"
			...
			<c:forEach items="${tl}" var="t" varStatus="st">
				...
				<!----------------- 传值向listTalkDetail.jsp --------------->
						<form name="${t.id}" id="${t.id}" action="listTalkDetail" method="get">  
							<input type="hidden" name="id" id="id" value="${schoolbaid}"/>  
							<input type="hidden" name="talkid" id="talkid" value="${t.id}"/>
							<input type="hidden" name="userid" id="userid" value="${userid}"/>  
							<a style="font-size: 25px;" onclick="document.getElementById('${t.id}').submit();">${t.title}</a>  
						</form>
							<%-- <a href="listTalkDetail?id=${schoolbaid}&talkid=${t.id}&userid=${userid}">${t.title}</a> --%>
						</h4>
					</div>
